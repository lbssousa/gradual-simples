% !TeX root = ../fasciculo-3/a4.tex
% chktex-file 1

\subsection{Entrada}\label{subsection:tempus-per-annum/missa-1/introitus}
\MakeChantAntiphonPsalm{index-cantuum/antiphonae-ad-introitum/}{omnis-terra.4E/}

\subsection[Salmo Responsorial]{Salmo Responsorial \textmd{C 2 a}}\label{subsection:tempus-per-annum/missa-1/psalmus-responsorius}
\MakeChantPsalmOneVerse{index-cantuum/psalmi-responsorii/}{iubilate-deo-omnis-terra.C2a/}

\AllowPageFlush

\subsection{Aleluia}\label{subsection:tempus-per-annum/missa-1/alleluia}
\MakeChantAntiphonPsalm{index-cantuum/alleluia/}{dominus-regnavit-exsultet-terra.8c/}

\subsection[Salmo Aleluiático]{Salmo Aleluiático \textmd{E \protect\GreStar}}\label{subsection:tempus-per-annum/missa-1/psalmus-alleluiaticus}
\MakeChantPsalmOneVerse{index-cantuum/psalmi-alleluiatici/}{dominus-regnavit-exsultet-terra.ES/}

\subsection{Ofertório}\label{subsection:tempus-per-annum/missa-1/offertorium}
\MakeChantAntiphonPsalm{index-cantuum/antiphonae-ad-offertorium/}{iubilate-deo.7a/}

\AllowPageFlush

\subsection{Comunhão}\label{subsection:tempus-per-annum/missa-1/communio}
\MakeChantAntiphonPsalm{index-cantuum/antiphonae-ad-communionem/}{mirabantur-omnes.1f/}