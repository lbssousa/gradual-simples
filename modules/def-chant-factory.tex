...
\NewDocumentCommand \SetVersePairs { m } {
  \iftoggle{onecolumn}{
    \begin{greenumerate}
      \setcounter{enumi}{1}
      \MakeVerseListR{#1}
...
\NewDocumentCommand \SectionAlleluiaPsalm { O{} m m m } {
  ...
  \MakeChantAlleluia{#2}
  \nobreaksubsection{#3}
  \MakeChantPsalm{psalmi-ad-alleluia}{#4}[\begin{rubrica}Todos:\end{rubrica} Allelúia.][\begin{rubrica}Todos:\end{rubrica} Aleluia.]
}

\NewDocumentCommand \SectionPsalm { O{} m O{} m m m } {
  \subsection{Salmo #2 --- #4}\IfBlankF{#3}{\label{#3}}
  \IfBlankF{#1}{%
    \begin{rubrica}
      \begin{center}
        #1
  ...
  \MakeChantPsalm{#5}{#6}[\Responsorium]
}
...
\NewDocumentCommand \SectionPsalmMulti { O{} m O{} m m m m } {
  ...
  \MakeChantPsalmMulti{#5}{#6}{#7}[\Responsorium]
}
